...
def write_default_config_file():
    config['DATABASE'] = {'Location': database_location,'Compression': 'TODO', 'CompressionLevel': 'TODO'}
    with open(config_file, 'w') as configfile:
        config.write(configfile)
    return True

def read_config_file(config_file=config_file):
    if os.path.isfile(config_file) and is_non_zero_file(config_file):   #race condition, config.read throws no error when it trys to read empty or non existing config file...
